...
_ReadRegionArgType = Tuple[Tuple[int, int], int, Tuple[int, int]]
...
class DeepZoomGenerator:
    ...
    def __init__(
        ...
        if limit_bounds:
            ...
            self._l0_offset = (0, 0)
        self._l0_dimensions = cast(Tuple[int, int], self._l_dimensions[0])
        # Deep Zoom level
        z_size = self._l0_dimensions
        z_dimensions = [z_size]
        while z_size[0] > 1 or z_size[1] > 1:
            z_size = cast(
                Tuple[int, int], tuple(max(1, int(math.ceil(z / 2))) for z in z_size)
            )
    ...
    def _get_tile_info(
    ...
    ) -> Tuple[_ReadRegionArgType, Tuple[int, int]]:
        # Check parameters
        ...
        z_size = cast(
            Tuple[int, int],
            tuple(
                min(self._z_t_downsample, z_lim - self._z_t_downsample * t)
                + z_tl
                + z_br
    ...
    def get_dzi(self, format: Union[Literal["png"], Literal["jpeg"]]) -> str:
